...
that group's component permissions. Full detail in [`webservices-api.md`](webservices-api.md) § Authentication and Tokens.
...
Registering in a bootstrap that only runs for the HTML document (or that calls
`$app->getDocument()` unguarded, which fatals under the console) will not cover
the API or console applications — register from the component's `boot()` instead.
